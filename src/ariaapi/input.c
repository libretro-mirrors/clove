...
static void keyboard_init(void) {
  ...
  moduleData.mouse_x= 0;
  moduleData.mouse_y = 0;
...
static SDL_Keycode keyboard_getKeycode(char const* name) {
  for(int i = 0; i < moduleData.numKeys; ++i) {
...
void ar_input_update(ar_State* S)
{
  ...
  while(SDL_PollEvent(&aria_input_event)) {
      ...
      switch (aria_input_event.type) {
        ...
        case SDL_KEYDOWN:
          {
            int nk = normalizeKeyCode(aria_input_event.key.keysym.sym);
            if(nk < moduleData.numKeys) {
                moduleData.keystate[nk] = true;
                if (aria_input_event.key.repeat == 0)
                  ar_call_global_s(S, "love-keypressed", ar_new_number(S, nk));
              }
          }
          break;
        case SDL_KEYUP:
          {
            ...
            if(nk < moduleData.numKeys) {
                ...
                if (aria_input_event.key.repeat == 0)
                  ar_call_global_s(S, "love-keyreleased", ar_new_number(S, nk));
              }
          }
          break;
        case SDL_TEXTINPUT:
...
ar_Value* ar_keyboard_isDown(ar_State* S, ar_Value* args, ar_Value* env)
{
  bool any = false;
  ...
  while (args)
    {
      ...
      int nk = normalizeKeyCode(keyboard_getKeycode(key));
      any = any || nk < moduleData.numKeys && moduleData.keystate[nk];
...
void ar_input_register(ar_State* S)
{
  ...
  {
  {"love:keyboard-isDown", ar_keyboard_isDown},
